...
public class ModelManager implements ModelService {
	...
	public void delete(int id) {
		this.businessModelRules.checkIfModelId(id);
		this.modelRepository.deleteById(id);
		
	}

	@Override
	public List<GetAllModelResponse> findBrandId(int brandId) {
		
		List<Model>models=this.modelRepository.findByBrandId(brandId);
		List<GetAllModelResponse>allModelResponses=models.stream().map(model->this.modelMapperService.forResponse().map(model,GetAllModelResponse.class)).collect(Collectors.toList());
